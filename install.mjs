...
/**
 * ShipBuilder CLI Installer
 * 
 * A beautiful installer for the ShipBuilder CLI.
 * 
 * Usage: curl -fsSL https://shipbuilder.app/install.mjs | node -
 */
...
function printBanner() {
  // Same structure as TUI: SHIP in cyan, BUILDER in brightPurple
  const lines = [
    { ship: '███████╗██╗  ██╗██╗██████╗ ', builder: '██████╗ ██╗   ██╗██╗██╗     ██████╗ ███████╗██████╗ ' },
    { ship: '██╔════╝██║  ██║██║██╔══██╗', builder: '██╔══██╗██║   ██║██║██║     ██╔══██╗██╔════╝██╔══██╗' },
    { ship: '███████╗███████║██║██████╔╝', builder: '██████╔╝██║   ██║██║██║     ██║  ██║█████╗  ██████╔╝' },
  ...
  for (const line of lines) {
    console.log(`  ${c.cyan}${line.ship}${c.brightPurple}${line.builder}${c.reset}`);
  }
  console.log();
}
...
async function getLatestVersion() {
  ...
  try {
    ...
    const npmResponse = await fetch('https://registry.npmjs.org/@shipbuilder/cli/latest');
    if (npmResponse.ok) {
...
function printNpmNotFoundError() {
  ...
  console.log(`${c.error}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${c.reset}`);
  console.log();
  console.log(`  ${c.dimGray}ShipBuilder requires npm to install pnpm.${c.reset}`);
...
async function installCLI(version) {
  ...
  const spinner = isQuietMode ? null : new Spinner('Installing ShipBuilder CLI...').start();
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      if (code === 0) {
        if (spinner) spinner.success('ShipBuilder CLI installed');
        resolve();
      } else {
...
function verifyInstallation() {
  try {
    ...
    const output = execSync('shipbuilder --version', { 
      encoding: 'utf8',
      env: { ...process.env, SHIPBUILDER_SKIP_UPDATE_CHECK: '1' },
      stdio: ['pipe', 'pipe', 'pipe'],
    }).trim();
    ...
    log(`  ${S.success} Verified: ${c.dim}shipbuilder v${version}${c.reset}`);
    return true;
  } catch {
...
function printSuccess() {
  ...
  console.log();
  console.log(`    ${c.cyan}shipbuilder${c.reset}          Launch the interactive TUI`);
  console.log(`    ${c.cyan}shipbuilder init${c.reset}     Initialize configuration`);
  console.log(`    ${c.cyan}shipbuilder --help${c.reset}   Show all commands`);
  console.log();
}
...
function printFailure(error) {
  ...
  if (error.stderr?.includes('heap out of memory') || error.stderr?.includes('ENOMEM')) {
    ...
    console.log(`    ${c.cyan}NODE_OPTIONS="--max-old-space-size=8192" pnpm add -g @shipbuilder/cli${c.reset}`);
  } else {
    console.log(`  ${c.dimGray}Try manual installation:${c.reset}`);
    console.log(`    ${c.cyan}pnpm add -g @shipbuilder/cli${c.reset}`);
  }
  console.log();
...
const isQuietMode = process.env.SHIPBUILDER_QUIET_INSTALL === '1';
...
async function main() {
  ...
  if (!isQuietMode) {
    printBanner();
    console.log(`  ${c.dim}Installing ShipBuilder CLI${c.reset}`);
    console.log();
  }

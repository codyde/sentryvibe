...
export interface RunnerConfig {
  version: string;
  workspace: string;
  monorepoPath?: string; // Path to cloned shipbuilder repository
...
export class ConfigManager {
  ...
  constructor() {
    this.conf = new Conf<RunnerConfig>({
      projectName: 'shipbuilder',
      projectSuffix: '', // No suffix, just 'shipbuilder'
  ...
  private getDefaults(): RunnerConfig {
    return {
      version: '0.1.0',
      workspace: join(homedir(), 'shipbuilder-workspace'),

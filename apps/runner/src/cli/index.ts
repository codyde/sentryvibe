...
//
// NOTE: @shipbuilder/agent-core is bundled directly into dist/ by tsup,
...
const isLinkedDevelopment = packageRoot.includes('/shipbuilder/apps/runner');
...
if (!isLinkedDevelopment) {
  ...
  const nodeModulesDir = dirname(packageRoot); // Go up from package to node_modules/@shipbuilder
...
// Sentry instrumentation is loaded via --import flag in bin/shipbuilder.js wrapper
...
const isRunCommand = args[0] === 'run'; // `shipbuilder run` uses TUI Dashboard
const isRunnerCommand = args[0] === 'runner' && !args.includes('--no-tui'); // `shipbuilder runner` uses TUI Dashboard (unless --no-tui)
const isVersionCommand = args.includes('--version') || args.includes('-V'); // Skip banner for version output
const isSkipBanner = process.env.SHIPBUILDER_SKIP_BANNER === '1'; // Skip banner after auto-update restart
...
if (!process.env.SHIPBUILDER_SKIP_UPDATE_CHECK && !isVersionCommand) {
  ...
  try {
    if (isTUIMode) {
      ...
      if (updateInfo?.updateAvailable) {
        ...
        process.env.SHIPBUILDER_UPDATE_AVAILABLE = updateInfo.latestVersion;
      }
    } else {
...
program
  .name('shipbuilder')
  .description('ShipBuilder CLI - AI App Builder')
...
program
  .command('runner')
  .description('Start runner only (connect to ShipBuilder server)')
  .option('-u, --url <url>', 'ShipBuilder server URL (default: https://shipbuilder.up.railway.app)')
  .option('-w, --workspace <path>', 'Workspace directory (default: ~/shipbuilder-workspace)')

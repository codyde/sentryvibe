...
export async function startCommand(options: StartOptions) {
  ...
  s.start('Locating ShipBuilder repository');
  ...
  const nextBuildIdPath = join(monorepoRoot, 'apps', 'shipbuilder', '.next', 'BUILD_ID');
  ...
  try {
    ...
    const envLocalPath = join(monorepoRoot, 'apps', 'shipbuilder', '.env.local');
    const envContent = [
      '# Auto-generated by shipbuilder CLI - DO NOT EDIT',
      `# Generated at: ${new Date().toISOString()}`,
      '',
      `SHIPBUILDER_LOCAL_MODE=${isLocalMode ? 'true' : 'false'}`,
    ...
    const webApp = spawn('pnpm', ['--filter', 'shipbuilder', webCommand], {
      cwd: monorepoRoot,

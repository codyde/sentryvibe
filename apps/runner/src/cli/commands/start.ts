...
export async function startCommand(options: StartOptions) {
  ...
  // In local mode, SQLite is used by default - no DATABASE_URL needed!
  // Only require DATABASE_URL if user explicitly configured one (hosted mode)
  // The MODE env var will be set based on whether databaseUrl exists
  ...
  // Determine database mode: LOCAL (SQLite) if no DATABASE_URL, HOSTED (PostgreSQL) if set
  const databaseMode = config.databaseUrl ? 'HOSTED' : 'LOCAL';
  
  const envContent = [
    '# Auto-generated by sentryvibe CLI - DO NOT EDIT',
    `# Generated at: ${new Date().toISOString()}`,
    '',
    '# Database mode: LOCAL uses SQLite, HOSTED uses PostgreSQL',
    `MODE=${databaseMode}`,
    '',
    ...
    databaseMode === 'HOSTED' ? `DATABASE_URL=${config.databaseUrl}` : '# DATABASE_URL not needed in LOCAL mode (SQLite)',
    '',
  ].join('\n');
  ...
  // Only set DATABASE_URL if in HOSTED mode (PostgreSQL)
  if (databaseMode === 'HOSTED' && config.databaseUrl) {

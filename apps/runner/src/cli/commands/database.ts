...
export async function databaseCommand() {
  ...
  if (!monorepoPath) {
    ...
    logger.info('Run "shipbuilder init" first to set up the repository');
    process.exit(1);
  }
  ...
  if (!databaseUrl) {
    ...
    logger.log(`  ${chalk.cyan('shipbuilder config set databaseUrl <url>')}`);
    process.exit(1);
  }
  ...
  if (!pushed) {
    ...
    logger.log(`  ${chalk.cyan('cd apps/shipbuilder')}`);
  ...
  logger.info(`You can now run: ${chalk.cyan('shipbuilder run')}`);
  logger.log('');
}

  ...
  switch (action) {
    case 'get':
      if (!key) {
        ...
        logger.info('Usage: sentryvibe-cli config get <key>');
        process.exit(1);
      }
      handleGet(key);
      break;
    ...
    case 'set':
      if (!key || !value) {
        ...
        logger.info('Usage: sentryvibe-cli config set <key> <value>');
        process.exit(1);
      }
      handleSet(key, value);
      break;
...
async function handleReset() {
  ...
  try {
    ...
    logger.info('Run "sentryvibe-cli init" to reconfigure');
  } catch (error) {

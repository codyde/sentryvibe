...
async function executeInitFlow(
  ...
  // Build packages
  startTask('build', '@shipbuilder/agent-core');
  try {
  ...
  try {
    ...
    // Create .env.local
    if (monorepoPath) {
      const envLocalPath = join(monorepoPath, 'apps', 'shipbuilder', '.env.local');
      const envContent = [
        '# Auto-generated by shipbuilder CLI',
        `# Generated at: ${new Date().toISOString()}`,
        '',
        'SHIPBUILDER_LOCAL_MODE=true',

...
const BROKER_URL = process.env.RUNNER_BROKER_URL ?? 'ws://localhost:4000/socket';
const SHARED_SECRET = process.env.RUNNER_SHARED_SECRET;
const HEARTBEAT_INTERVAL_MS = 15_000;

// Create the instrumented Claude query (Sentry wrapper for Claude Code SDK)
...
async function handleCommand(command: RunnerCommand) {
  ...
  switch (command.type) {
    ...
    case 'start-dev-server': {
      try {
        const { runCommand: runCmd, workingDirectory, env = {}, preferredPort, framework } = command.payload;
      ...
      try {
        ...
        while (true) {
          ...
          if (typeof value === 'string') {
            ...
            console.warn('Unsupported chunk type from build stream:', typeof value);
            continue;
          }

          if (!loggedFirstChunk) {
            console.log('[runner] first build chunk sample:', JSON.stringify(agentMessage).slice(0, 200));
        ...
        if (finalChunk) {
          let payload = finalChunk.startsWith('data:') ? finalChunk : `data: ${finalChunk}`;
          if (!payload.endsWith('\n\n')) {
            payload = `${payload}\n\n`;
          }
          sendEvent({

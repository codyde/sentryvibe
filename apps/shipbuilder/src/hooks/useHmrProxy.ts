 ...
 * 
 * Flow:
 * 1. Iframe posts 'shipbuilder:hmr:connect' message
...
export function useHmrProxy({
  ...
  const handleIframeMessage = useCallback((event: MessageEvent) => {
    ...
    if (!type?.startsWith('shipbuilder:hmr:')) return;
    
    switch (type) {
      case 'shipbuilder:hmr:connect':
        handleHmrConnect(event.data);
        break;
      case 'shipbuilder:hmr:send':
        handleHmrSend(event.data);
        break;
      case 'shipbuilder:hmr:disconnect':
        handleHmrDisconnect(event.data);
        break;
      case 'shipbuilder:hmr:ready':
        ...
        sendToIframe('shipbuilder:hmr:config', { port: devServerPort });
        break;
    }
    ...
    try {
      ...
      switch (message.type) {
        case 'hmr-connected':
          ...
          sendToIframe('shipbuilder:hmr:connected', { connectionId: message.connectionId });
          break;
          
        case 'hmr-message':
          sendToIframe('shipbuilder:hmr:message', { 
        ...
        case 'hmr-closed':
          ...
          sendToIframe('shipbuilder:hmr:closed', {
            connectionId: message.connectionId,
            code: message.code,
        ...
        case 'hmr-error':
          ...
          activeConnectionsRef.current.delete(message.connectionId);
          sendToIframe('shipbuilder:hmr:error', {
            connectionId: message.connectionId,
            error: message.error,
          });
          break;

      ...
      if (onRenameProject) {
        items.push({
          ...
          action: {
            type: 'action',
            fn: () => {
              onRenameProject({ id: project.id, name: project.name || 'Untitled Project' });
              onOpenChange(false);
            },
          },
          group: 'Manage',
      ...
      if (onDeleteProject) {
        items.push({
          ...
          action: {
            type: 'action',
            fn: () => {
              onDeleteProject({ id: project.id, name: project.name || 'Untitled Project', slug: project.slug });
              onOpenChange(false);
            },
          },
          group: 'Manage',
    ...
    projects.forEach((project) => {
      ...
      if (bulkMode) {
        ...
        items.push({
          ...
          label: `${isSelected ? '✓ ' : ''}${project.name || 'Untitled Project'}`,
        ...
        {selectedProject && (
          <div className="flex items-center gap-2 px-4 py-2 border-b border-white/10 bg-purple-500/10">
            ...
            <span className="text-sm text-white font-medium">
              {projects.find(p => p.id === selectedProject)?.name || 'Untitled Project'}
            </span>
          </div>
        )}

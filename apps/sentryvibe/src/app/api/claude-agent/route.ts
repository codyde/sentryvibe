            ...
            if (block.name === 'Bash' || block.name === 'Read' || block.name === 'Write' || block.name === 'Edit') {
              ...
              if (typeof pathToCheck === 'string') {
                ...
                if (pathToCheck.includes('/Users/') || pathToCheck.includes('/home/')) {
                  ...
                  console.error(`   Expected CWD: ${workspaceDir}`);
...
1. Use ONLY relative paths from your CWD (${workspaceDir})
...
❌ cd ${workspaceDir}/my-app
❌ ls ${workspaceDir}/my-app
...
3. After completion, you may test the build:
  - If needed, start the dev server using the PORT environment variable provided by the system (never hardcode a port)

...
const GITHUB_INSTALL_SCRIPT_URL =
  "https://raw.githubusercontent.com/codyde/sentryvibe/main/install-cli.sh";
...
export async function GET() {
  try {
    const response = await fetch(GITHUB_INSTALL_SCRIPT_URL, {
      headers: {
        "User-Agent": "SentryVibe-Install-Proxy",
      },
    ...
    if (!response.ok) {
      ...
      return new NextResponse(
        "# Error: Failed to fetch install script\necho 'Failed to download install script. Please try again or visit https://github.com/codyde/sentryvibe'\nexit 1",
        {
          status: 502,
          headers: {
